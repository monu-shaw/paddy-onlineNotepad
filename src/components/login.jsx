...
export default function Login({ network }) {
  ...
  const LogIn = async () => {
    ...
    querySnapshot.docs.map((e) => e.data()).length !== 0
      ? dispatch(logIN(querySnapshot.docs.map((e) => e.id)))
  ...
  return (
    <>
      ...
      <Typography
        ...
        component="p"
        align="center"
        className="mb-4 fixed-bottom col-11 col-md-9 col-lg-7 mx-auto nunito"
      >
        An Online Notepad For use To Copy All important notes. Specialy Design
        For Developer So Do not need to copy data on Every Devices. Chats are
        For Chatting Not For Copy and pasting. Try Today Paddy , Your Perfect
        Development Parnter.
      </Typography>
    </>
  );
